...
$chat_grup = mysqli_query($conn, "SELECT chat_grup.smester as smester, pengumuman.judul as topik FROM chat_grup JOIN pengumuman ON chat_grup.id_pengumuman = pengumuman.id");

$super_user = query("SELECT * FROM super_user WHERE username = '$username'")[0];
$id_wali = $super_user['id'];

$grup = mysqli_query($conn, "SELECT * FROM chat_grup WHERE id_penerima != $id_wali GROUP BY id_penerima");

$total_mhs = mysqli_num_rows($grup);
...
<h2>Daftar Hadir</h2> -->
  ...
  <br>
		...
		<table border="1" cellspacing="0" cellpadding="0">
			...
			<?php $cg = mysqli_query($conn, "SELECT * FROM chat_grup"); ?>
			<?php $i=1; ?>
			<?php foreach ($cg as $g) : ?>
